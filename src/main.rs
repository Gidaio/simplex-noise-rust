...
fn simplex_noise(gradients: &[Vector], sample_position: Vector) -> f64 {
    let skew_factor = (sample_position.x + sample_position.y) * (3f64.sqrt() - 1.0) / 2.0;
    let skewed_sample_position = Vector {
        x: sample_position.x + skew_factor,
        y: sample_position.y + skew_factor,
    };

    let simplex_vertex_0 = Vector {
        x: skewed_sample_position.x.floor(),
        y: skewed_sample_position.y.floor(),
    };
    let unskew_factor = (1.0 - 1.0 / 3f64.sqrt()) / 2.0;
    let unskewed_vertex_0 =
        simplex_vertex_0 - (simplex_vertex_0.x + simplex_vertex_0.y) * unskew_factor;
    let distance_0 = sample_position - unskewed_vertex_0;

    let simplex_vertex_1 = if distance_0.x > distance_0.y {
        Vector {
            x: simplex_vertex_0.x + 1.0,
            y: simplex_vertex_0.y,
        }
    } else {
        Vector {
            x: simplex_vertex_0.x,
            y: simplex_vertex_0.y + 1.0,
        }
    };
    let simplex_vertex_2 = Vector {
        x: simplex_vertex_0.x + 1.0,
        y: simplex_vertex_0.y + 1.0,
    };

    let gradient_0 = gradients[((simplex_vertex_0.x + simplex_vertex_0.y * GRID_HEIGHT as f64)
        % (GRID_WIDTH * GRID_HEIGHT) as f64) as usize];
    let gradient_1 = gradients[((simplex_vertex_1.x + simplex_vertex_1.y * GRID_HEIGHT as f64)
        % (GRID_WIDTH * GRID_HEIGHT) as f64) as usize];
    let gradient_2 = gradients[((simplex_vertex_2.x + simplex_vertex_2.y * GRID_HEIGHT as f64)
        % (GRID_WIDTH * GRID_HEIGHT) as f64) as usize];

    let unskewed_vertex_1 =
        simplex_vertex_1 - (simplex_vertex_1.x + simplex_vertex_1.y) * unskew_factor;
    let unskewed_vertex_2 =
        simplex_vertex_2 - (simplex_vertex_2.x + simplex_vertex_2.y) * unskew_factor;
